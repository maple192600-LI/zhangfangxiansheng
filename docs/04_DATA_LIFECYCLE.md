...
|--------|------|--------------|------------------|
| 结果预览行 | `待确认` / `异常` | `business_date`、`entity_code`、`account_code` 可暂缺，供用户在预览页修正 | 否 |
| 正式基础数据行 | `正常` | `business_date`、`entity_code`、`account_code` 必须完整 | 是 |

因此，导入阶段不能用空字符串假装匹配成功；无法匹配的单位/账户编码应保持为空并在结果预览中标记异常。提交前必须重新校验，只有全部核心字段完整的行才能从 `待确认` 改为 `正常`。
...
→ account_resolution_attempts / account_resolution_evidence（留痕归属判断）
→ SourceFile.status 更新（ready / needs_rule / needs_account）
→ import_preview_service 统一预览/编辑/校验
→ parser 成功：SourceFile.status → parsed
→ parser 失败：SourceFile.status → failed + error_code
→ commit → FundEvent(state=正常)
```

- 完整链路参见 [`14_BANK_IMPORT_GENERALIZATION.md`](14_BANK_IMPORT_GENERALIZATION.md)
- `SourceFile` 记录文件级处理状态（parser 匹配、解析成功/失败），`FundEvent` 记录行级流水数据，二者职责不同
- 上传阶段已接入 identity/bank/account/parser 匹配（09D）
- 结果预览阶段使用 bank/format 匹配路径（`build_bank_import_context`），负责解析落入 `FundEvent(state=待确认)`、编辑异常、最终提交
- 最终提交不重新 parser，不覆盖用户编辑结果（09D2）
- `import_preview_service.commit()` 统一 bank/manual 提交：验证 → 状态变更为 `正常`
...
- 快速录入生成批次和待确认 FundEvent，跳转统一结果预览页
- 校验通过后通过 `import_preview_service.commit()` 提交为正式数据
- 如果单位、账户或日期无法识别，仍应进入结果预览，不应因数据库约束导致整批失败
...
- 手工 Excel 不再通过 `manual-flow/commit` 公开入口提交；最终提交统一走结果预览页
...
**最后校准：** 2026-05-19（SourceFile + account resolution 接入）
