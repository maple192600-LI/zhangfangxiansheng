...
→ ParserArtifact 匹配
→ bank_import_service.preview() 解析预览
→ 用户确认进入上传结果预览
→ import_preview_service 统一预览/编辑/校验/提交
→ FundEvent(state=正常)
```
...
- `artifact_runtime.run_parser` 已实现（底层执行器），ParserArtifact 可真实解析 xlsx。但银行格式识别和账户归属匹配尚未实现，仍需后续交付
