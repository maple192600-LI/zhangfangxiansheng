...
code_entry: ""                               # 旧体系字段（fund_skill_run 已删除）。新技能应通过通用 Agent 工具与 artifact service / artifact runtime 契约完成
...
format_skill_instruction(skill)
  ...
  → 注意：fund_skill_run 已在 Phase 5 删除，统一走 skill_run
  ↓
LLM 调用 skill_run 工具
  ↓
...
- [x] ~~fund_skill_run 经验记录~~（Phase 5 已删除）
