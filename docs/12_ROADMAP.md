...
| 4 | 前端 placeholder 清理 | 移除 当前禁止进入主流程的 placeholder 路由 |
...
- [x] 12C：规则中心与现有 Agent 体系衔接 + CI 修复（PR #64 追加 commit）— 移除创建智能体入口、修正空状态文案、修正 starter prompt、修复 test_schema_tables CI、前端响应解包修复
...
- [ ] 移除 当前禁止进入主流程的 placeholder 路由（OCR、贷款、预算，共 9 个）
...
---
**校准来源：** `backend/core/artifact_runtime.py`
