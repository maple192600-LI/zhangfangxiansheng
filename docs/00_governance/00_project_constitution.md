...
>
> 相关文档：
> - [项目文档入口](../README.md) · docs/README.md
> - [数据库契约](../30_contracts/20_database_schema.md) · 31 张表 DDL
> - [API 契约](../30_contracts/23_api_contracts.md) · 端点清单 + 错误码
> - [基元库白名单](../30_contracts/25_primitives_whitelist.md) · Parser/Rule 可调用函数
...
> **注意**：上述能力方向不代表固定的 5 个 skill 绑定。技能由通用 Agent 统一调度，不得绑定到领域专用调度器。旧 Fund Agent 目录已删除。
...
详细能力配置见 [Agent 系统](../07_AGENT_SYSTEM.md)。
...
银行导入不是只做流水字段清洗，还必须读取主数据管理信息，把流水准确归属到法人单位和银行账户。

- Parser 只负责银行格式解析，不负责最终账户归属决策
- 账户归属匹配是独立服务能力，与 Parser Runtime 解耦
- ParserArtifact `kind="bank"` 目标是银行/格式级，不是账户级
- 没有事实线索（无账号、无单位/户名）时，不能假装智能识别，不能默认第一个账户
- 完整契约见 [`../14_BANK_IMPORT_GENERALIZATION.md`](../14_BANK_IMPORT_GENERALIZATION.md)
- 账户归属判断必须留痕到 `account_resolution_attempts` / `account_resolution_evidence`，供结果预览解释

---

## §C6 · 数据库 · 当前活动表集合

以下为当前 `backend/db/tables.py` 中 `__tablename__` 定义的全部 31 张业务 ORM 表。`sqlite_sequence` 为 SQLite 系统表，不计入业务表。
...
25. agent_messages                 ← Agent 消息（含工具调用和推理内容）
...
已移除：旧 Agent 配置表 — 无对应 ORM 类，功能合并入当前 Agent 实例表。

合计 31 张业务 ORM 表。完整 DDL 见 [`../30_contracts/20_database_schema.md`](../30_contracts/20_database_schema.md) §T1-§T8。
...
API inventory 来源：`python tools/guards/check_api_inventory.py --list`
当前为 166 effective endpoints，0 duplicate route identities。端点数量不是固定上限；`check_api_inventory.py` 检测 effective path 重复而非数量。后续新增端点需更新 `23_api_contracts.md`。
...
**校准来源：** `backend/db/tables.py`、`backend/agents/runtime.py`、`backend/fund/primitives/`、`tools/guards/`
**最后校准：** 2026-05-18
