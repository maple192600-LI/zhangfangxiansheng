...
>
> 相关文档：
> - [01_scope_and_order.md](01_scope_and_order.md) · 范围与执行顺序
> - [08_anti_drift.md](08_anti_drift.md) · 防跑偏六层机制
> - [09_agent_capability.md](09_agent_capability.md) · Agent 能力体系
...
现金日记账是当前必须交付的基础报表模板。其 Excel 模板的占位符集合冻结如下：
...
当前预置以下 5 个财务技能，Agent 可在此基础上通过 `skill_creator` 动态创建新技能：
...
详细能力配置见 [09_agent_capability.md](09_agent_capability.md)。
...
API 端点约束见 [`../30_contracts/23_api_contracts.md`](../30_contracts/23_api_contracts.md)。

当前 API inventory guard 使用迁移基线阻止端点继续膨胀。后续处理必须优先合并、删除或兼容迁移旧入口；不得继续新增端点扩大混乱。每完成一批端点收敛后，必须下调 guard 基线。
