...
5. account_aliases
6. manual_field_pool
7. manual_template_schemes
8. import_batches
9. fund_events                    ← CANONICAL_12（§C1）
10. daily_report_runs
11. ai_configs
12. ai_call_logs
13. agent_configs
14. operation_logs
15. users
16. report_templates
17. parser_artifacts               ← ParserArtifact 银行/手工解析器
...
25. skills_v2                      ← 技能注册表
26. sqlite_sequence                ← SQLite 自增序列（系统表）
27. (预留)
```

合计 27 表（含 1 系统表）。完整 DDL 见 [`../30_contracts/20_database_schema.md`](../30_contracts/20_database_schema.md) §T1-§T7。
