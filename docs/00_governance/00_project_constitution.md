...
> - [数据库契约](../30_contracts/20_database_schema.md) · 28 张表 DDL
...
以下为当前 `backend/db/tables.py` 中 `__tablename__` 定义的全部 28 张业务 ORM 表。`sqlite_sequence` 为 SQLite 系统表，不计入业务表。
...
合计 28 张业务 ORM 表。完整 DDL 见 [`../30_contracts/20_database_schema.md`](../30_contracts/20_database_schema.md) §T1-§T8。
...
**版本**
- v5.0 · 2026-05-17 · 顶部相关文档指向 active docs（移除旧文档死链接）；§C4 引用指向 07_AGENT_SYSTEM.md；§C7 API 口径改为 166 effective endpoints + 无上限 + guard 检测重复
- v4.5 · 2026-05-13 · §C6 工作流编排改为 workflows / workflow_versions / workflow_runs / workflow_run_steps 四表版本化模型
- v4.4 · 2026-05-13 · §C6 增加 workflow_definitions / workflow_runs / workflow_run_steps 三张工作流编排表
- v4.3 · 2026-05-10 · §C4 重写为单通用 Agent 技能体系；§C6 产物表统一为 Agent 产物；§C8 补充 artifact runtime 确定性执行说明；新增 §C4.1 单通用 Agent 原则
- v4.2 · 2026-05-10 · 修正 §C6 表清单为当前 24 张业务 ORM 表（sqlite_sequence 不计入业务表，旧 Agent 配置表无 ORM 不计入，parser_templates 已移除）
