...
| **FundAgent** | ~~已删除（Phase 5，2026-05-11）~~。旧体系：独立的 Python 类，不继承 Agent，不调用 LLM，只做 5 个固定 skill 的确定性执行。ParserArtifact / RuleArtifact 的生成和维护现由通用 Agent 完成 | ~~`backend/agents/fund/harness.py:35`~~ | 无专属表 | ~~财务调度器~~ | **已删除** |
...
| `FundAgent` ≠ 通用 Agent | FundAgent 已在 Phase 5 删除，通用 Agent 是唯一的 Agent 实体。`backend/agents/fund/` 已删除 ≠ `backend/fund/`（产物执行基础设施，保留） |
...
| P2 | 旧 `FundAgent` 类已删除（Phase 5），可复用逻辑已迁移到 `backend/services/` | 旧 FundAgent 体系已删除 | 低 — 已完成 | Phase 5 已完成 |
