...
agents/       ← Agent 运行时：通用 Agent + 工具注册 + 会话管理（`agents/fund/` 是旧中间态，待迁移后删除）
config.py     ← 全局配置
...
│   ├── harness.py     ← 旧 FundAgent 调度器（待删除）
│   ├── schemas.py     ← 输入/输出 Schema（待迁移）
│   ├── memory.py      ← 样本库/字段字典访问层（待迁移）
...
- `backend/agents/fund/` 是旧 FundAgent 中间态，待迁移后删除；`backend/fund/` 是产物确定性执行基础设施，必须保留
