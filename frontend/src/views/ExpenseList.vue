  ...
  <div>
    <div class="section">
      ...
      <table>
        <thead>
          <tr>
            <th>日期</th><th>法人</th><th>账户</th><th>摘要</th><th>对方</th><th>支出金额</th><th>余额</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="r in rows" :key="r.id">
            ...
            <td class="money exp">{{ fmtAmt(r.amount) }}</td>
            <td class="money balance">{{ fmtAmt(r.rolling_balance) }}</td>
          </tr>
          <tr v-if="!rows.length"><td colspan="7" style="text-align:center;color:var(--muted);padding:30px">暂无支出数据</td></tr>
        </tbody>
      </table>
    </div>
