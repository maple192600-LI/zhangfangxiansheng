...
export const useAgentsStore = defineStore('agents', () => {
  ...
  async function fetchAll() {
    loading.value = true
    try {
      list.value = await http.get('/agent/agents')
    } catch (e) {
  ...
  async function createAgent(data) {
    const result = await http.post('/agent/agents', data)
    await fetchAll()
    return result
  }

  async function updateAgent(id, data) {
    const result = await http.put(`/agent/agents/${id}`, data)
    await fetchAll()
    return result
  }

  async function deleteAgent(id) {
    await http.delete(`/agent/agents/${id}`)
    await fetchAll()
  }

  async function getAgent(id) {
    return await http.get(`/agent/agents/${id}`)
  }

  async function fetchAIConfigs() {
    return await http.get('/agent/ai-configs')
  }

  async function createSession(agentId, title) {
    return await http.post(`/agent/agents/${agentId}/sessions`, { title })
  }

  async function listSessions(agentId) {
    return await http.get(`/agent/agents/${agentId}/sessions`)
  }

  async function getMessages(sessionId) {
    return await http.get(`/agent/sessions/${sessionId}/messages`)
  }

  async function listFiles(agentId, subDir = 'workspace') {
    return await http.get(`/agent/agents/${agentId}/files`, { params: { sub_dir: subDir } })
  }

  async function uploadFile(agentId, file, subDir = 'inbox') {
    const formData = new FormData()
    formData.append('file', file)
    return await http.post(`/agent/agents/${agentId}/files/upload`, formData, {
  ...
  async function listMemories(agentId) {
    return await http.get(`/agent/agents/${agentId}/memories`)
  }

  async function saveMemory(agentId, key, content) {
    return await http.post(`/agent/agents/${agentId}/memories`, { key, content })
  }
