...
- **Artifact runtime**：`run_parser` 已实现（ParserArtifact deterministic runtime，底层执行器）；`run_rule` 仍为 `NotImplementedError`（Phase H1 待交付）。ParserArtifact 可创建、审核、执行。RuleArtifact 可创建和审核，但无法通过 artifact runtime 执行。银行格式识别和账户归属匹配尚未实现。
